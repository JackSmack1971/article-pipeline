...
**Citation format:** Phrase links only. Wrap the natural phrase in prose as `[anchor text](https://absolute-url)`. Source URLs are drawn from `claims_for_drafting.md` only when that row's `URL Status` is `AVAILABLE`. When `URL Status` is `URL-MISSING`, the table must show `—` in `Source URL`; cite by attribution only and never substitute a publisher homepage, database homepage, search URL, or guessed URL. Use "Suggested Anchor" as a starting point — rewrite to fit the sentence naturally. T3 sources: append `†` to anchor: `[anchor text†](url)`. `[BREAKING-UNVERIFIED]` claims: phrase link + `*(Editorial note: Verify before publication.)*` on the next line. `[NAMED-ENTITY-UNVERIFIED]` claims: no link, remove named entity. No `[^N]` markers, no footnote block, no `[Source: ...]` text anywhere in the draft.
